...
void HestonSLVModelTest::testLocalVolsvSLVPropDensity() {
    ...
    const Handle<BlackVolTermStructure> vTS = Handle<BlackVolTermStructure>(createSmoothImpliedVol(dayCounter, calendar).get<2>());
    ...
    const boost::shared_ptr<HestonProcess> hestonProcess(boost::make_shared<HestonProcess>(rTS, qTS, spot, v0, kappa, theta, sigma, rho));

    const Handle<HestonModel> hestonModel(boost::make_shared<HestonModel>(hestonProcess));

    const Handle<LocalVolTermStructure> localVol(boost::make_shared<NoExceptLocalVolSurface>(vTS, rTS, qTS, spot, 0.3));
    ...
    for (std::list<HestonSLVFDMModel::LogEntry>::const_iterator iter= logEntries.begin(); iter != logEntries.end(); ++iter) {
        ...
        if (t > 0.2) {
            ...
            const std::vector<Real>& z= iter->mesher->getFdm1dMeshers().at(1)->locations();

            const boost::shared_ptr<Array>& prob = iter->prob;

            for (Size i=0; i < z.size(); ++i) {
                const Real pCalc = DiscreteSimpsonIntegral()(x, Array(prob->begin()+i*xGrid,prob->begin()+(i+1)*xGrid));
...
test_suite* HestonSLVModelTest::experimental(SpeedLevel speed) {
    test_suite* suite = BOOST_TEST_SUITE("Heston Stochastic Local Volatility tests");
