...
test_suite* LiborMarketModelProcessTest::suite(SpeedLevel speed) {
    ...
    suite->add(QUANTLIB_TEST_CASE(LiborMarketModelProcessTest::testInitialisation));
    suite->add(QUANTLIB_TEST_CASE(LiborMarketModelProcessTest::testLambdaBootstrapping));

    if (speed <= Fast) {
        suite->add(QUANTLIB_TEST_CASE(LiborMarketModelProcessTest::testMonteCarloCapletPricing));
    }
