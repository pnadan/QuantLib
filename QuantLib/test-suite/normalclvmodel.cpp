...
test_suite* NormalCLVModelTest::experimental(SpeedLevel speed) {
    ...
    suite->add(QUANTLIB_TEST_CASE(NormalCLVModelTest::testBSCumlativeDistributionFunction));
    suite->add(QUANTLIB_TEST_CASE(NormalCLVModelTest::testHestonCumlativeDistributionFunction));
    suite->add(QUANTLIB_TEST_CASE(NormalCLVModelTest::testIllustrative1DExample));
    suite->add(QUANTLIB_TEST_CASE(NormalCLVModelTest::testMonteCarloBSOptionPricing));

    if (speed == Slow) {
        suite->add(QUANTLIB_TEST_CASE(NormalCLVModelTest::testMoustacheGraph));
    }
